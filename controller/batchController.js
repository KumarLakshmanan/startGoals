...
export const createBatchWithAutoEnrollment = async (req, res) => {
  ...
  try {
    ...
    if (autoEnrollmentCriteria) {
      const { enrollmentStatus, registrationDate, skillLevel } = autoEnrollmentCriteria;
    ...
    if (recurringPattern) {
      ...
      while (currentDate <= end) {
        ...
        const liveSession = await LiveSession.create({
          ...
          description: sessionDescription || `Recurring session for ${batch.title}`,
...
export const getBatchAnalytics = async (req, res) => {
  try {
    ...
    const batch = await Batch.findByPk(batchId, {
      include: [
        ...
        },
        {
          model: LiveSession,
          as: 'sessions',
          attributes: ['sessionId', 'title', 'scheduledStartTime', 'actualStartTime', 'actualEndTime', 'status']
        }
      ]
    });
    ...
    const sessionStats = {
      ...
      inProgress: batch.sessions.filter(s => s.status === 'in_progress').length,
    ...
    const completedSessions = batch.sessions.filter(s => s.actualStartTime && s.actualEndTime);
    const avgSessionDuration = completedSessions.length > 0
      ? completedSessions.reduce((sum, session) => {
          const duration = new Date(session.actualEndTime) - new Date(session.actualStartTime);
          return sum + duration;
      ...
      data: {
        ...
        statistics: {
          enrollment: {
            ...
            enrollmentRate: batch.maxStudents > 0 ? (students.length / batch.maxStudents) * 100 : 0
          },
          sessions: {
            ...sessionStats,
    ...
    const batch = await Batch.findByPk(batchId, {
      include: [
        ...
        },
        {
          model: LiveSession,
          as: 'sessions',
          attributes: ['sessionId', 'title', 'scheduledStartTime', 'status', 'actualStartTime', 'actualEndTime']
        }
      ]
    });
