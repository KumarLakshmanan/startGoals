  ...
  try {
    ...
    const mockAdminUsers = [
      {
        ...
        permissions: [
          'manage_users', 'manage_courses', 'manage_projects', 'manage_payments', 
          'manage_certificates', 'manage_settings', 'view_analytics', 'manage_admins'
        ],
        status: 'active',
      ...
      },
      {
        ...
        permissions: [
          'manage_courses', 'view_analytics', 'manage_instructors', 'moderate_content'
        ],
        status: 'active',
      ...
      },
      {
        ...
        permissions: [
          'manage_projects', 'view_analytics', 'manage_downloads', 'moderate_content'
        ],
        status: 'active',
      ...
      },
      {
        ...
        permissions: [
          'manage_payments', 'view_analytics', 'manage_refunds', 'view_financial_reports'
        ],
        status: 'active',
      ...
      },
      {
        ...
        permissions: [
          'manage_certificates', 'view_analytics', 'generate_reports'
        ],
        status: 'inactive',
    ...
    let filteredAdmins = mockAdminUsers.filter(admin => {
      ...
      if (search) {
        ...
        return admin.firstName.toLowerCase().includes(searchLower) ||
    ...
    const paginatedAdmins = filteredAdmins.slice(offset, offset + parseInt(limit));

    // Calculate analytics
    const analytics = {
      totalAdmins: totalCount,
      activeAdmins: filteredAdmins.filter(a => a.status === 'active').length,
      inactiveAdmins: filteredAdmins.filter(a => a.status === 'inactive').length,
      suspendedAdmins: filteredAdmins.filter(a => a.status === 'suspended').length,
      roleDistribution: {
        super_admin: filteredAdmins.filter(a => a.adminRole === 'super_admin').length,
        course_manager: filteredAdmins.filter(a => a.adminRole === 'course_manager').length,
        project_manager: filteredAdmins.filter(a => a.adminRole === 'project_manager').length,
        payment_manager: filteredAdmins.filter(a => a.adminRole === 'payment_manager').length,
        certificate_manager: filteredAdmins.filter(a => a.adminRole === 'certificate_manager').length
      },
      avgActivityScore: filteredAdmins.length > 0 ? 
        (filteredAdmins.reduce((sum, a) => sum + a.activityScore, 0) / filteredAdmins.length).toFixed(1) : 0,
      totalLoginCount: filteredAdmins.reduce((sum, a) => sum + a.loginCount, 0),
      onlineAdmins: filteredAdmins.filter(a => 
        new Date() - new Date(a.lastActivity) < 30 * 60 * 1000 // 30 minutes
      ).length
    };
...
export const createAdminUser = async (req, res) => {
  try {
    ...
    if (!['super_admin', 'course_manager', 'project_manager', 'payment_manager', 'certificate_manager'].includes(adminRole)) {
    ...
    if (invalidPermissions.length > 0) {
      validationErrors.push(`Invalid permissions for role: ${invalidPermissions.join(', ')}`);
    }
    ...
    const newAdmin = {
      ...
      isAdmin: true,
      adminRole,
      permissions: permissions.length > 0 ? permissions : getRolePermissions(adminRole),
      status,
      createdAt: new Date(),
      updatedAt: new Date(),
      createdBy,
...
export const updateAdminUser = async (req, res) => {
  try {
    ...
    if (updateData.adminRole && !['super_admin', 'course_manager', 'project_manager', 'payment_manager', 'certificate_manager'].includes(updateData.adminRole)) {
      validationErrors.push("Invalid admin role specified");
    }
    ...
    if (updateData.permissions && updateData.adminRole) {
      ...
      const invalidPermissions = updateData.permissions.filter(p => !rolePermissions.includes(p));
      if (invalidPermissions.length > 0) {
        validationErrors.push(`Invalid permissions for role: ${invalidPermissions.join(', ')}`);
      }
    }
...
export const deleteAdminUser = async (req, res) => {
  try {
    ...
    if (userRole === 'super_admin' && superAdminCount <= 1 && !forceDelete) {
      return res.status(400).json({
        success: false,
        message: "Cannot delete the only super admin. Create another super admin first or use forceDelete=true.",
    ...
    const hasActiveActivities = Object.values(activeActivities).some(count => count > 0);

    if (hasActiveActivities && !transferActivities && !forceDelete) {
      return res.status(400).json({
        success: false,
        message: "Admin has active activities. Please transfer activities to another admin or use forceDelete=true.",
  ...
  try {
    ...
    const analytics = {
      ...
      actionDistribution: {
        ...
        approvals: filteredLogs.filter(l => l.action.includes('approved')).length,
        deletions: filteredLogs.filter(l => l.action.includes('deleted')).length,
        creations: filteredLogs.filter(l => l.action.includes('created')).length
      },
      moduleDistribution: {
        ...
        authentication: filteredLogs.filter(l => l.module === 'authentication').length,
        admin_management: filteredLogs.filter(l => l.module === 'admin_management').length
      },
...
export const getRolePermissions = (role = null) => {
  const allPermissions = {
    super_admin: [
      ...
      // Course Management
      'manage_courses', 'view_courses', 'create_courses', 'update_courses', 'delete_courses', 'approve_courses',
      // Project Management
      'manage_projects', 'view_projects', 'create_projects', 'update_projects', 'delete_projects', 'approve_projects',
      // Payment Management
      'manage_payments', 'view_payments', 'process_refunds', 'view_financial_reports',
      // Certificate Management
      'manage_certificates', 'view_certificates', 'generate_certificates', 'revoke_certificates',
      // Admin Management
      'manage_admins', 'view_admins', 'create_admins', 'update_admins', 'delete_admins',
      // Settings Management
      'manage_settings', 'view_settings', 'update_settings', 'backup_settings',
      // Analytics
      'view_analytics', 'export_data', 'generate_reports',
      // Content Moderation
      'moderate_content', 'approve_content', 'reject_content'
    ],
    course_manager: [
      'manage_courses', 'view_courses', 'create_courses', 'update_courses', 'approve_courses',
      'manage_instructors', 'view_instructors', 'approve_instructors',
      'moderate_content', 'approve_content', 'reject_content',
      'view_analytics', 'generate_reports'
    ],
    project_manager: [
      'manage_projects', 'view_projects', 'create_projects', 'update_projects', 'approve_projects',
      'manage_downloads', 'view_downloads',
      'moderate_content', 'approve_content', 'reject_content',
      'view_analytics', 'generate_reports'
    ],
    payment_manager: [
      'manage_payments', 'view_payments', 'process_refunds',
      'view_financial_reports', 'export_financial_data',
      'manage_discounts', 'view_discounts',
      'view_analytics'
    ],
    certificate_manager: [
      'manage_certificates', 'view_certificates', 'generate_certificates', 'revoke_certificates',
...
export const getPermissions = async (req, res) => {
  try {
    ...
    const roleHierarchy = {
      super_admin: { level: 5, description: 'Full system access and admin management' },
      course_manager: { level: 3, description: 'Course and instructor management' },
      project_manager: { level: 3, description: 'Project and download management' },
      payment_manager: { level: 4, description: 'Payment and financial management' },
  ...
  try {
    ...
    switch (type) {
      ...
      default:
        return res.status(400).json({
          success: false,
          message: "Invalid export type. Supported types: users, activity_logs, permissions"
        });
    }
 ...
 * Log admin activity
 */
async function logAdminActivity(userId, action, module, details, targetId = null, targetType = null, ipAddress = null, risk = 'low') {
