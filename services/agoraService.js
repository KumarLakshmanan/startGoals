...
class AgoraService {
  ...
  generateToken(channelName, userId, role, expirationTimeInSeconds) {
    ...
    const privilegeExpiredTs =
      currentTimestamp + (expirationTimeInSeconds || 3600); // Default to 1 hour
  ...
  async createLiveSession(sessionDetails) {
    try {
      ...
      const channelName =
        sessionDetails.channelName || `agora_session_${Date.now()}`;
  ...
  async controlParticipantMic(channelName, targetUserId, mute) {
    ...
    console.log(
      `AgoraService: Setting mic for user ${targetUserId} in channel ${channelName} to ${mute ? "muted" : "unmuted"}`,
    );
  ...
  async controlParticipantCamera(channelName, targetUserId, disable) {
    ...
    console.log(
      `AgoraService: Setting camera for user ${targetUserId} in channel ${channelName} to ${disable ? "disabled" : "enabled"}`,
    );
  ...
  async removeParticipant(channelName, targetUserId) {
    ...
    console.log(
      `AgoraService: Removing user ${targetUserId} from channel ${channelName}`,
    );
    // Example: agoraEngine.kickUser(targetUserId, channelName); // Fictional method, actual API might differ
    return {
      success: true,
      userId: targetUserId,
      message: "Participant removed from Agora session",
    };
  }
  ...
  async startCloudRecording(channelName) {
    ...
    return {
      success: true,
      message: "Cloud recording started (placeholder)",
